...
<head>
    ...
    <style>.colonel-table .introduce img {
        width: 100% !important;
        float: left;
...
<div class="my-colonel ae">
    ...
    <div class="custom newcustom">
        <div data-am-widget="slider" class="am-slider am-slider-a1 am-no-layout am-addol"
             data-am-slider="{&quot;directionNav&quot;:false}">
            <ul class="am-viewport" style="overflow: hidden; position: relative;">
                <ul class="am-slides"
                    style="width: 1200%; transition-duration: 0.6s; transform: translate3d(-640px, 0px, 0px);">
                    <li class="clone c-page" aria-hidden="true" style="width: 320px; float: left; display: block;">
                        <div class="colonel-pic ae"><img
                                src="http://pro.efeiyi.com/${group.groupProduct.productModel.productModel_url}@!wap-product-pic">

                            <div class="c-page">
                                <span>${group.groupProduct.productModel.product.name}[${group.groupProduct.productModel.name}]</span>
                            </div>
                        </div>
                    </li>
                    <c:forEach items="${group.groupProduct.productModel.product.productPictureList}" var="picture">
                        <c:if test="${picture.status=='1'&&picture.productModel.id==group.groupProduct.productModel.id}">
                            <li class="clone c-page" aria-hidden="true"
                                style="width: 320px; float: left; display: block;">
                                <div class="colonel-pic ae"><img
                                        src="http://pro.efeiyi.com/${picture.pictureUrl}@!wap-product-pic">
    ...
    <div class="iwill ae">
        <div class="join ae">
            <div class="txt-page ae">
                <span>团长:${manUserName}</span><span>${group.groupMemberList.size()}人参团/${group.groupProduct.memberAmount}人起成团</span>
            </div>
            ...
            <c:if test="${flag==1}">
                <c:if test="${group.groupProduct.memberAmount-group.groupMemberList.size()>0}">
                    <a href="javascript:void(0)" class="btn"
                       id="btn">还&nbsp;差&nbsp;${group.groupProduct.memberAmount-group.groupMemberList.size()}&nbsp;人&nbsp;成&nbsp;团&nbsp;,&nbsp;去&nbsp;分&nbsp;享</a>
                </c:if>
                <c:if test="${group.groupProduct.memberAmount-group.groupMemberList.size()<=0}">
        ...
        <div class="c-content ae">
            <div class="co-page">
                <div class="introduce ae">
                    ...
                    <div class="button ae"><a
                            href="http://www.efeiyi.com/product/productModel/${group.groupProduct.productModel.id}"
                            class="gbtn"><span>原价直接购买</span><i class="icon1"></i></a></div>
                </div>
                <%--<div class="tab-btn">--%>
                            ...
                            <c:forEach items="${purchaseOrderProductList}" var="purchaseOrderProduct" varStatus="rec">
                                <c:if test="${not empty purchaseOrderProduct.purchaseOrderComment&&purchaseOrderProduct.purchaseOrderComment.status!='0'}">
                                    <li>
                                        ...
                                        <div class="co-pic"><img class="am-circle"
                                                                 src="<c:url value="/scripts/wap/upload/yonghm.jpg"/>">
                                        </div>
                                        <h5>${fn:substring(user, 0,3 )}****${fn:substring(user,7,11)}</h5>

                                        <P>${purchaseOrderProduct.purchaseOrderComment.content}</P>
