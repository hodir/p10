    ...
    <c:if test="${group.groupProduct.status==1}">
        <c:if test="${flag==0}">
            <c:if test="${group.status==1}">
                <h1 class="am-header-title"><i
                        class="had-icon had-add"></i>组团中，还差${group.groupProduct.memberAmount-group.groupMemberList.size()}人成团
                </h1>
            </c:if>
...
<div class="newshare ae">
    ...
    <div class="had-product ae">
        <div class="had-pic"><a
                href="<c:url value='/group/shareGroup.do'/>?groupProductId=${group.groupProduct.id}&groupId=${group.id}&memberId=${memberId}&show=1&purchaseOrderId=${purchaseOrderId}"><img
                src="http://pro.efeiyi.com/${group.groupProduct.productModel.productModel_url}@!group-share-product-view"></a>
        </div>
        <div class="had-txt">
            <p>${group.groupProduct.productModel.product.name}[${group.groupProduct.productModel.name}]</p>

            <p>${group.groupProduct.memberAmount}人起成团/${group.groupProduct.groupPurchaseTime}天成团</p>

            <p><strong>拼团价${group.groupProduct.groupPrice}</strong>（<s>原价${group.groupProduct.productModel.price}</s>）
            </p></div>
    </div>
    <!--成团人数-->
    <div class="new-team ae">
        <div class="old"><a href="#" class="iste"><img class="addnewbd"
                                                       src="<c:url value="/scripts/wap/upload/fx-x-1.png"/>"><strong>长</strong></a>
        </div>
        <div class="crew">
            <c:forEach items="${group.getGroupMemberList()}" var="groupMember">
                <c:if test="${groupMember.level!=0}">
                    <a href="#" class="iste"><img class="addnewbd"
                                                  src="<c:url value="/scripts/wap/upload/fx-x-1.png"/>"></a>
                </c:if>
            </c:forEach>
        </div>
